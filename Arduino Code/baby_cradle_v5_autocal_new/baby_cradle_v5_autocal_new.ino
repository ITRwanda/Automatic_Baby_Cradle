...
const int   CRY_MARGIN    = 2;     // only used if USE_MANUAL_THRESHOLD is false

// Set true to force CRY_THRESHOLD to a fixed number (ignores baseline math
// below). Your baseline drifts a bit boot-to-boot (438-445 seen so far),
// so this is the reliable way to pin it to an exact value like 440.
const bool  USE_MANUAL_THRESHOLD = true;
const int   MANUAL_THRESHOLD     = 440;
...
void calibrateSound() {
  ...
  if (USE_MANUAL_THRESHOLD) {
    CRY_THRESHOLD = MANUAL_THRESHOLD;
    Serial.printf("[Calibrate] Manual threshold override active: %d\n",
                  CRY_THRESHOLD);
  } else {
