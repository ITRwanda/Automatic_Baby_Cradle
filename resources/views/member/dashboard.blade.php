...
<div class="cgd-page">
    ...
    <div class="cgd-header">
        <div>
            ...
            <p class="cgd-sub">Welcome back, {{ auth()->user()->name }}</p>
        </div>
        <div style="display:flex; gap:8px; flex-wrap:wrap;">
            <a href="{{ route('caregiver.reports') }}" class="cgd-btn-primary">
            ...
            <a href="{{ route('caregiver.notifications') }}" class="cgd-btn-outline">
                ...
                Notifications
                @if($unreadCount > 0)
                    <span class="u-badge">{{ $unreadCount }}</span>
                @endif
            </a>
        </div>
    </div>
    ...
    <div class="cgd-kpi">
        ...
        <div class="cgd-kpi-card k-sky">
            ...
            <div class="cgd-kpi-value">{{ $notifications->count() }}</div>
    ...
    <div class="cgd-grid">
        ...
        <div class="cgd-panel">
            <div class="cgd-panel-hdr" style="background:linear-gradient(135deg,#0f172a,#1e293b);color:#fff;">
                ...
                @if($unreadCount > 0)
                    <span class="u-badge">{{ $unreadCount }} new</span>
                @endif
            </div>
            <div class="cgd-panel-body" style="padding:14px 16px;">
                @forelse($notifications as $note)
                    ...
                    @endphp
                    <div class="notif-mini {{ $cls }}">
                        <div class="notif-mini-title">{{ $icon }} {{ $note->title }}</div>
                        <div class="notif-mini-body">{{ Str::limit($note->body, 100) }}</div>
                        <div class="notif-mini-time">
                            {{ $note->created_at?->diffForHumans() }}
                            @if($note->device) &nbsp;·&nbsp; {{ $note->device->device_name }} @endif
                            @if(!$isUnread) &nbsp;·&nbsp; <span style="color:#15803d;">✓ read</span> @endif
                        </div>
                    </div>
                @empty
                ...
                <div style="margin-top:12px;text-align:center;">
                    <a href="{{ route('caregiver.notifications') }}" class="cgd-btn-outline" style="font-size:.78rem;padding:6px 14px;">
